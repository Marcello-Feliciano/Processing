/************************************************************
/* Author: STUDENT NAME: Marcello Feliciano
/* Permission to share in Rohrbach Library and YouTube (YES or NO): YES
/* Creation Date: 11/21-28/2022
/* Due Date: Thursday 12/15/2022
/* Course: CSC220
/* Professor Name: Dr. Parson
/* Assignment: 5.
/* Purpose: Create video for installation with credit in Rohrbach Library & YouTube.
/* See https://faculty.kutztown.edu/parson/fall2022/csc220fall2022assn5.html for requirements.
*************************************************************/
/*  KEYBOARD COMMANDS
  ...
  e[LOWER[,UPPER]] set loadImage etch threshold between LOWER and UPPER inclusive,
    when UPPER not given it is 0 trhough LOWER, when neither shut it off, i.e., both -1.
  ...
  +bINT                filter(BLUR,INT), TAKES 1 NUMERIC ARGUMENT FOR BLUR FILTER
  +fINT,FLOAT          fade [0, 255] = [transparent, opaque], TAKES 2 ARGS for int opacity & float opacityDecayRate
  +eFLOAT              etch with probability range [0.0, 1.0], TAKES 1 NUMERIC ARGUMENT FOR PROBABILITY OF ETCH
  +nFLOAT              nest with scale range (0.0, 1.0), TAKES 1 NUMERIC ARGUMENT FOR SCALE-DOWN OF AMOUNT
  +m                   mask image 0 with image 1
  +s                   blend SCREEN image 0 with image 1
  +tFLOAT              threshold etcher with initial threshold set to FLOAT
...
final String CREDIT = "Marcello Feliciano, CSC220, Fall 2022";
final String KeyRecordingFile = "Assn5KeyRecording.txt" ; // "Assn5KeyRecording.txt" ; // Make null to skip recording/playback
...
void setup() {
  ...
  try {
    ...
    System.err.println("ERROR: CSC220Fall2022PaintingAssn5.txt not found.");
    exit();
  }
...
void drawBackground() {
  ...
  rect(-width/4-width/8, 0, 50, 50);
  fill(255, 0, 255);
  circle(width/4+width/8, 0, 50);
  fill(255, 0, 0);
  ellipse(0, -height/4-height/8, 50, 100);
  fill(0, 255, 0);
  triangle(0, height/4+height/8, 0, height/5+height/10,100,360);
  pop();
}
