...
/* Due: via D2L Assignment 3 at 11:59 PM Thursday November 10.
/* Course: CSC220 Object-Oriented Multimedia Programming
/* Professor Name: Dr. Parson
/* Assignment: 3.
/* Sketch name: CSC220F22Assn3_MIDI (derived from CSC220F22Assn2_3D).
...
/*    Save a working copy of this sketch as CSC220F22Assn3_MIDI WITH YOUR NAME at the top.
/*
 *********************************************************/
...
PImage cloverPImage = null ;
PShape cloverPShape = null ;
...
void setup() {
  ...
  // above loadImage call, allowing customPImageParson to be null.
  ...
  avatars[1] = new NewAvatar(3*width/4, 3*height/4, 0, 2, 1, 1, 1);
  avatars[2] = new VectorAvatar(int(random(0,width)), int(random(0,height)), 0, 3, 4, round(random(2,5)), 0.5);
  avatars[3] = null ;  // STUDENT 1 construct your Vector here.
  ...
  for (int i = avatars.length/2-2 ; i < avatars.length ; i++) {
      //avatars[1] = new NewAvatar(3*width/4, 3*height/4, 0, 2, 1, 1, 1);
      //int(random(random(0,width)), int(random(0, height));
  }
 ...
 **/
  ...
  **/
  ...
  /** Added for assn3, mow() removes its HayBale object from avatars[], no effect otherwise. **/
  void mow();
}
