...
class Professor extends CollisionDetector {
  ...
  void move() {
    ...
    if (colliders.size() > 0) {
      ...
      int controller_modwheel = 1 ;
      int modLocation = int(constrain(map(myy, 0, width, 0, 127),0,127));  // 64 is centered in stereo field
      sendMIDI(ShortMessage.CONTROL_CHANGE, channel, controller_modwheel, modLocation);
      int chorusControl = 93 ;
      int chorusLocation = int(constrain(map(myz, 0, width, 0, 127),0,127));  // 64 is centered in stereo field
      sendMIDI(ShortMessage.CONTROL_CHANGE, channel, chorusControl, chorusLocation);
    ...
    for (Avatar other : colliders) {
      ...
      if (probability < 90) { // mow 90% of them changed from 60
        other.mow();
        //add sound here
      }
    }
...
class VectorAvatar extends CollisionDetector {
  ...
  void move() {
    ...
    for (Avatar other : colliders) {
      float probability = random(0, 100);
      if (probability < 70) { // mow() 70% changed from 40% of them 
        other.mow();
      }
    }
  }
  int [] getBoundingBox() {
    ...
    */
    result[0] = round(myx - 100 * myscale) ; //round(myx-myscale*customPShapeParson.width/2.0);
    result[1] = round(myy - 100 * myscale) ; //round(myy-myscale*customPShapeParson.height/2.0);
    result[2] = round(myz - 100 * myscale) ; //round(myz-myscale*customPShapeParson.depth/2.0);
    result[3] = round(myx + 100 * myscale) ; //round(myx+myscale*customPShapeParson.width/2.0);
    result[4] = round(myy + 100 * myscale) ; //round(myy-myscale*customPShapeParson.width/2.0);
    result[5] = round(myz + 100* myscale) ; // round(myz-myscale*customPShapeParson.depth/2.0);
    return result ;
  }
}
