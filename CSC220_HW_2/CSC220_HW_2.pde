/************************************************************
/* Authors: Dr. Parson & (Marcello Feliciano)
/* Creation Date: 11/14/2016, updated 9/23/2019, 9/25/2020, 9/22/2021, 9/21/2022
/* Due Date: ??? - We will have a lab class, start before then.
/* Course: CSC220 Object-Oriented Multimedia Programming
/* Professor Name: Dr. Parson
/* Assignment: 2.
/* Sketch name: CSC220F22Assn2_3D
...
/*    1. 50% Completely replace my Professor class with your class that uses 3D boxes & spheres;
...
/*       TO GET STARTED You could rename Professor to your class name globally, run it,
...
void setup() {
  ...
  customPImage = loadImage("arrow_use"); // load before makeCustomPShape(), USE YOUR OWN
  ...
  avatars[0] = new NewAvatar(width/4, height/4, 0, 2, 3, 1, 2);
  ...
  avatars[5] = new NewAvatar(3*width/4, 3*height/4, 0, 2, 1, 1, 1);
  ...
  avatars[12] = new VectorAvatar(int(random(0,width)), int(random(0,height)), 0, 3, 4, round(random(2,5)), 1);
  for (int i = 13 ; i < avatars.length-1 ; i++) { // STUDENT BE CAREFUL TO START i AT *YOUR* NEXT OPEN SLOT!
    ...
    avatars[i] = new NewAvatar(int(random(0,width)), int(random(0, height)),
...
PShape makeCustomPShape(PImage textureimg) {
  ...
  front.beginShape();
  if (textureimg == null) {
    front.vertex(0,0,0);      // vertex(x, y, z) with varying
    front.vertex(0,100,0);  // Z did not work for texturing,
    front.vertex(100,100,0);   // even though the constant Y value
    front.vertex(100,0,0);   // even though the constant Y value
    front.vertex(0,0,0);      // makes this surface planar.
    
    front.endShape();
    front.setFill(color(255,255,255));
  } else {
    int imgwidth = round(textureimg.width);  // Use 2D vertex() calls to
    int imgheight = round(textureimg.height); // get texture to work.
    front.texture(textureimg);
    front.vertex(0,100,imgwidth/2,imgheight-1); // The u,v coordinates
    front.vertex(0,0,0,0);  // in the vertex() calls tell where in
    front.vertex(100,-100,imgwidth-1,0); // the PImage to attach to the
    front.endShape();  // PShape vertex.
    front.rotateX(radians(90));  // rotate with apex pointing at me
    front.translate(0,100,0);    // translate to drop it down to the floor.
  ...
  left.beginShape();
  left.vertex(0,0,0);      // center,bottom,front
  left.vertex(0,0,-100);       // center,top, center (pyramid apex)
  left.vertex(0,100,-100);  // left,bottom,back
  left.vertex(0,100,0);      // close at original vertex for setFill to work
  left.vertex(0,0,0);
  left.endShape();
  left.setFill(color(255,0,0));
  PShape right = createShape();
  right.beginShape();
  right.vertex(100,0,0);
  right.vertex(100,0,-100);
  right.vertex(100,100,-100);
  right.vertex(100,100,0);
  right.vertex(100,0,0);;
  right.endShape();
