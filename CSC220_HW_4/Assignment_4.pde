...
final int fRate = 60 ; // frame rate constant.
...
PImage STUDENTIMAGE ;
PShape STUDENTSHAPE, head, body, arms, leg1, leg2;  // STUDENT I set this via createShape in setup() but use this in drawShape().
...
void setup() {
  ...
  STUDENTIMAGE = loadImage("art_for_assignment_4.jpg");
  STUDENTIMAGE.resize(210, 0);
  STUDENTSHAPE = createShape(GROUP);
  /*head = createShape(ELLIPSE, -10550, 10450, 18000, 19000);
  head.setFill(color(255));
  body = createShape(RECT, 2500, 0, 5000, 400);
  body.setFill(color(0));
  arms = createShape(RECT, 500, 0, 1000, 1000);
  arms.setFill(color(0));
  leg1 = createShape(RECT, 2500, 0, 400, 400);
  leg1.setFill(color(0));
  leg2 = createShape(RECT, 2500, 0, 400, 400);
  leg2.setFill(color(0));
  STUDENTSHAPE.setFill(color(255, 255, 255,255));
  STUDENTSHAPE.setStroke(false);*/
  
   //rotateY(PI/2);
  head = createShape(ELLIPSE,-1700, 0, 700, 500);
  //rotateY(PI/2);
  //body = createShape(ELLIPSE, 900, 0, 500, 500);
  body = createShape(RECT,-1500, -100, 1000, 200);
  //rotateX(PI);
  leg1 = createShape(RECT, -200, -100, -300,50);
  leg2 = createShape(RECT, -200, 50, -300,50);
  //rotateY(-PI/2);
  //ellipse(-600,0,500, 500);
  
  STUDENTSHAPE.addChild(body);
  STUDENTSHAPE.addChild(head);
  //STUDENTSHAPE.addChild(arms);
  STUDENTSHAPE.addChild(leg1);
  STUDENTSHAPE.addChild(leg2);
  ...
  loadImage("art_for_assignment_4.jpg");
}
...
void drawShape(int mydepth, final float rotation) {
  ...
  */
  ...
  stroke(0);
  pop();*/
  shape(STUDENTSHAPE, 0, 0, 500, 500);
  image(STUDENTIMAGE,-350,-100);
}


/*void STUDENTSHAPE(int mydepth, final float rotation) {
  // Since HSB uses 0..359 degrees, just like rotation,
  // use rotation for HUE as well. I am adding 175 so
  // it comes out as cyan when there is no rotation.
  stroke((175+rotation)%360,100,100);
      ...
      if (controlKey) {
        if (controlKey) {
          ...
          if (key == 'x' || (int(key) == 24 && ! shiftKey)) {
            println("x");
            xSpanLimit += spanIncr; 
            if(xSpanLimit > xSpanRange){
              xSpanLimit = xSpanRange;
            }
              
          } else if (key == 'y'|| (int(key) == 25 && ! shiftKey)) {
            ySpanLimit += spanIncr; 
            if(ySpanLimit > ySpanRange){
              ySpanLimit = ySpanRange;
            }
          } else if (key == 'z'|| (int(key) == 26 && ! shiftKey)) {
            zSpanLimit += spanIncr; 
            if(zSpanLimit > zSpanRange){
              zSpanLimit = zSpanRange;
            }
      ...
      if (altKey) {
        ...
        if (altKey) {
          ...
          if (key == 'x') {
            ...
            if(xScaleAmount > xSpanRange){
              xScaleAmount = xSpanRange;
            }
            println("x");
          }
          else if (key == 'y') {
            yScaleAmount += spanIncr; 
            if(yScaleAmount > ySpanRange){
              yScaleAmount = ySpanRange;
            }
          }
          else if (key == 'z') {
            zScaleAmount += spanIncr; 
            if(zScaleAmount > zSpanRange){
              zScaleAmount = zSpanRange;
            } 
          }
          else if (key == 'X') {
            xScaleAmount -= spanIncr; 
