...
/* Assignment: 1.
/* Sketch name: CSC220F22AvatarClassInAvatarRoom
...
void setup() {
  ...
  avatars = new Avatar [ 50 ] ;
  avatars[0] = new NewAvatar(width/4, height/4, 2, 3, 2);//big avatar
  ...
  avatars[5] = new NewAvatar(3*width/4, 3*height/4, 2, 1, 1);
  ...
  avatars[10] = new PulsingPaddle(width/2, height/2, barlength, 10, .5, orange); // add professors or avatars here
...
class Professor extends CollisionDetector {
  ...
  Professor(int avx, int avy, float spdx, float spdy, float avscale) {
...
class NewAvatar extends CollisionDetector {
  /* The data fields store the state of the Avatar. */
  protected int legdist = 0 ; // You can initialize to a constant here.
  NewAvatar(int avx, int avy, float spdx, float spdy, float avscale) {
    super(avx,avy,spdx,spdy,avscale,0,0,0);
    // Call the base class constructor to initialize its data fields,
    // then initialize this class' data fields.
    xlimitright = width ;
  ...
  void display() {
    ...
    fill(240, 150, 150);
    //fill(150, 255, 255);
    quad(-20,20,20,20,20, -20, -20, -20); // head, 0,0 is the conceptual center of the object.
    // An object rotates around its 0,0 point.
    /*quad(-5 , 0, 5 , 0, 10 , 40 , -10 , 40 ); // neck
    fill(100, 200, 130);  // professor gown
    quad(-10 , 10, 10 , 10, 20 , 90 , -20 , 90 ); // torso
    //ellipse(0, 60 , 40 , 80 ); // torso
    stroke(0);
    // stick arms & legs
    strokeWeight(8);
    line(0, 60 , -20 -abs(10-legdist) , 120 );  // left leg
    line(0, 60 , 20 +abs(10-legdist) , 120 );  // right leg
    //ellipse(0, 60 , 20 +abs(10-legdist) , 120 );  // right leg
    strokeWeight(5);
    line(0, 60 , -40 , 20 -2*abs(10-legdist) );   // left arm
    line(0, 60 , 40 , 20 +2*abs(10-legdist) );   // right arm
    strokeWeight(2);
    ...
    quad(-20,10,-20,-20,20, 10, 20, -20); // outside triangle part of glasses
    fill(0);
    triangle(0, 1 , -3, 5 ,3, 5 );  // nose
    //ellipse(0, 1 , 5 , 5 );  // nose
    arc(0, 10 , 20 , 10 , 0, PI); // mouth
  ...
  int [] getBoundingBox() {
    ...
    result[3] = myy + round(120*myscale) ; // bottom of legs
    return result ;*/
    int [] result = rotateBB(-35, -30, 30, 20, myrot, myscale, myscale, myx, myy);
      return result ;
    /*int [] result = rotateBB(-round(40*myscale),  - round(30*myscale), round(myscale*max(20 +abs(10-legdist),40)), round(120*myscale), myrot, myscale, myscale, myx, myy);
    return result ;*/
  }
}
...
class Furniture extends CollisionDetector {
  ...
  void display() {
    ...
    stroke(mycolor);
    strokeWeight(1);
    rect(0,0,mywidth,myheight);  // 0,0 is the center of the object.
    pop(); // STUDENT *MUST* use pop() last in display().
  }
  // The move() function updates the Avatar object's state.
...
class PulsingPaddle extends Furniture {
  public boolean growing = true;
  // Call base class constructor to initialize its fields,
  // then initialize fields added by this class (none presently),
  // and let limits on off-screen excursions.
  PulsingPaddle(int avx, int avy, int w, int h, float rotatespeed, int c) {
    super(avx, avy, w, h, c);
    rotspeed = rotatespeed ;
    xlimitright = 2 * width ;
    ylimitbottom = 2 * height ;
  ...
  void move() {
    ...
    while (myrot <= 360) { // scale the paddle up and down as it's rotating
      myrot += 360 ;
      myscale += .01;
      if(myscale <= .2){
        myscale +=.2;
